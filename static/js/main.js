...
$(function () {
    ...
    ***************************/
    ...
    let cursor = '.mil-cursor'; // Make sure this matches the actual selector for your cursor
    gsap.to(cursor, {
        duration: 0.2, // Correct syntax for duration
        scale: 0,
        ease: 'sine'
    });
    ...
    let cursor = '.mil-cursor'; // Ensure this matches your cursor's selector
    gsap.to(cursor, {
        duration: 0.2, // Correct GSAP duration syntax
        scale: 1,
        ease: 'sine',
    });
    ...
    ***************************/
    ...
    document.addEventListener("swup:contentReplaced", function () {
        ...
        ***************************/
        ...
        $('a:not(.mil-choose, .mil-more, .mil-drag, .mil-accent-cursor), input, textarea, .mil-accordion-menu').mouseover(function () {
    let cursor = '.mil-cursor'; // Define the cursor selector
    gsap.to(cursor, {
        duration: 0.2, // Use correct GSAP 3 syntax
        scale: 0,
        ease: 'sine',
    });
    gsap.to('.mil-ball svg', {
        ...
        $('a:not(.mil-choose, .mil-more, .mil-drag, .mil-accent-cursor), input, textarea, .mil-accordion-menu').mouseleave(function () {
    let cursor = '.mil-cursor'; // Define the cursor element
    gsap.to(cursor, {
        duration: 0.2, // Correct GSAP syntax for duration
        scale: 1,
        ease: 'sine',
    });
...
const io = new IntersectionObserver((entries) => {
  entries.forEach(entry => {
    const target = entry.target.querySelector('.icosahedron');
    if (target) {
...
document.querySelectorAll('.scene').forEach(scene => io.observe(scene));

const observerForScene  = new IntersectionObserver((entries) => {
  entries.forEach(entry => {
    const target = entry.target.querySelector('.icosahedron');
    if (target) {
      target.style.animationPlayState = entry.isIntersecting ? 'running' : 'paused';
